...
const HeroSection = () => {
  ...
  return (
    <section className="mt-4 space-y-6">
      {/* Main Hero Grid */}
      <div className="grid grid-cols-12 gap-4 lg:gap-6 md:h-[370px]">
          ...
          <nav className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-3 h-full flex flex-col justify-between shadow-lg shadow-black/5">
            <div>
              <div className="flex items-center gap-1.5 mb-1.5 pb-1.5 border-b">
                ...
                <h2 className="font-bold text-[10px] uppercase tracking-wider gradient-text">Danh mục</h2>
              </div>
              <ul className="space-y-1">
                {categories.map(({ label, icon: Icon, to, bgColor }, index) => (
                  <li key={label} style={{ animationDelay: `${index * 0.1}s` }} className="animate-fade-in opacity-0" >
                    <Link
                      to={to}
                      className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 transition-colors group"
                    >
                      <div className={`w-6 h-6 rounded-lg ${bgColor} flex items-center justify-center transition-all duration-300 group-hover:scale-110`}>
                        <Icon className="h-3 w-3" />
                      </div>
                      <span className="font-medium text-[11px] flex-1">{label}</span>
            ...
            <div className="pt-2 border-t">
              <div className="grid grid-cols-2 gap-1.5">
                <div className="text-center p-1.5 rounded-lg bg-gradient-to-br from-blue-500/10 to-purple-500/10">
                  <div className="text-xs font-bold gradient-text">500+</div>
                  <div className="text-[9px] text-muted-foreground">Sản phẩm</div>
                </div>
                <div className="text-center p-1.5 rounded-lg bg-gradient-to-br from-green-500/10 to-emerald-500/10">
                  <div className="text-xs font-bold text-green-600">5000+</div>
                  <div className="text-[9px] text-muted-foreground">Đã bán</div>
                </div>
              </div>
            </div>
          </nav>
